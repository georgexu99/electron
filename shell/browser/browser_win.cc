...
v8::Local<v8::Promise> Browser::CreateThumbnailFromPath(
  ...
  hr = SHCreateItemFromParsingName(image_path.c_str(), nullptr, IID_PPV_ARGS(&pItem));

  if (FAILED(hr)) {
    promise.RejectWithErrorMessage("Failed to create IShellItem from the given path");
    return handle;
  }

  //Init thumbnail cache 
  IThumbnailCache* pThumbnailCache = nullptr;
  hr = CoCreateInstance(CLSID_LocalThumbnailCache, nullptr, CLSCTX_INPROC, IID_PPV_ARGS(&pThumbnailCache));
  if (FAILED(hr)) {
    promise.RejectWithErrorMessage("Failed to acquire local thumbnail cache reference");
    pItem->Release();
    return handle;
  }
  ...
  hr = pThumbnailCache->GetThumbnail(pItem, size, WTS_FLAGS::WTS_NONE, &pThumbnail, &flags, &thumbId);
  pItem->Release();

  if(FAILED(hr)) {
    promise.RejectWithErrorMessage("Failed to get thumbnail from local thumbnail cache reference");
  ...
  image_skia.AddRepresentation(gfx::ImageSkiaRep(skbitmap, 1.0 /*scale factor*/));
  gfx::Image gfx_image = gfx::Image(image_skia);
  promise.Resolve(gfx_image);
  return handle;
  // free hicon n stuff check it!
}
